...
int main(int argc, char* argv[])
{
	...
	// create noise gen
	primitive::generator::impulse impulse_gen;

	// prepare
	impulse_gen.prepare(sample_rate, block_size);
	...
	while (samples_remaining > 0) {
		...
		impulse_gen.perform(output_buffer, samples_current, 0, samples_completed);
		impulse_gen.trigger();
	...
	// save
	helper::io::wav_file_save("test_impulse.wav", sample_rate, 32, output_buffer, 0);
